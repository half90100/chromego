...
//    点击到插件设置页面。
//
var BASE_URL = 'https://zhushou.huihui.cn/';
var FEED_URL = BASE_URL + "api/hui/latest.json?limit=60"; //折扣列表
var SHARE_URL = BASE_URL + "api/hui/share.json";
var ORDER_URL = BASE_URL + "/conf/orderparser2.json"; //顶部广告位
var NAVIGATION_URL = BASE_URL + "api/hui/navigation.json"; //顶部网站入口
...
var MINUNITE = 60 * 1000; //这里是获取数据的间隔时间60 * 1000
...
(function initLocalStorage() {
    ...
    // 顶部网站入口
    if (localStorage['push.bannerTxt'] === undefined) {
        localStorage['push.bannerTxt'] = [];
    }
    ...
    if (localStorage["push.isNotify"] === undefined) { //桌面弹框
    ...
    if (localStorage["push.figureRemind"] === undefined) { //数字提醒
...
function checkShareInfoApi(isRefresh, isFirst) { //晒物，白菜价数据
    var finalUrl;
    ...
    function isTodayNews(pubTime) {
        ...
        var thisDayDate =new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate(), 0).getTime();
...
(function getBannerNav() {
    simpleAjax(NAVIGATION_URL, function (msgs) {
        var msg = msgs || [];
        localStorage['push.bannerTxt'] = serialize(msg);
    });
}());

function checkPushMsgs(isRefresh, isFirst) { //折扣列表数据
    ...
    simpleAjax(finalUrl, function (msgs) {
        ...
        _.each(msgs, function (item) {
            // if (item.pubTime > startOfDay) { //之前逻辑一天前的数据不显示，新逻辑全显示
        ...
        //localStorage["push.lastMsgId"] = '44075000' //假数据
        var lastId = parseInt(localStorage["push.lastMsgId"], 10) || 0; //存储的最后一个id
        _.each(msgs, function (msg) {
            if (lastId > 0 && msg.id <= lastId || msg.pubTime <= lastDate) { //如果本msg的id大于存储的id，则视为新数据
                return;
            }

            var cfg = unserialize(localStorage.getItem("push.config"));
            var category = cfg ? cfg.category >> 0 : 0; //0：非订阅用户:push.hasFeeded=false
        ...
        if (figureRemind === "true") {
            ...
            //console.log('新数字',newCount)
            //console.log(111,newCount,notifyMsgsFun.length,todayFeeds.length)
            chrome.browserAction.getBadgeText({}, function (result) {
                var beforeCount = result >> 0;
                //console.log('原数字:',beforeCount)
                newCount += beforeCount;
                newCount = newCount > 0 ? newCount : '';
                newCount = newCount > 99 ? '99+' : newCount;
                //console.log('icon上的数字:',newCount)
                chrome.browserAction.setBadgeText({'text': newCount + ''});
                chrome.browserAction.setBadgeBackgroundColor({color: '#fe3c00'});
            });
        } else {
...
function getNotify(notifyMsgsFun, max, current) {
    //console.log(notifyMsgsFun, max, current)
...
function onInstalled() {
    ...
    //360浏览器捆绑安装不打开新窗口 LOG记录要唯一
    // sendLog(["action=INSTALL_CHROMIUM_360AUTO", "type=ARMANI_EXTENSION_EXINSTALL_360AUTO"]);
    // var o = new StrOpt(get("youdaoGWZS_jsOptions"));
    // o["state"] = "install";
    // set("youdaoGWZS_jsOptions", o.toOptString());
