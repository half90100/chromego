        ...
        function(response) {
            ...
            if (!on) {
                ...
                var href = location.href,
                    isAliCSP = !!(href.match(/^http(?:s)?:\/\/([a-zA-Z0-9-_]+\.)*(tmall|taobao)\./)) && !!(document.querySelector('meta[http-equiv=Content-Security-Policy]')),
                    // isVipCSP = !!(href.match(/^http(?:s)?:\/\/www\.vip\.com\/?(\?.*)?$/));
                    isVipCSP = !!(href.match(/^http(?:s)?:\/\/([a-zA-Z0-9-_]+)\.vip\.com\/?(.*)?(\?.*)?$/)),
                    isJumeiCSP = !!(href.match(/^http(?:s)?:\/\/([a-zA-Z0-9-_]+)\.jumei(?:[a-zA-Z0-9-_]+)?\.com\/?(.*)(\?.*)?$/));
                extension_js();
                // if(isAliCSP || isVipCSP || isJumeiCSP){
                //     extension_js();
                // }
                // else {
                //
                ...
                //         //本地
                //         // loadUrl = protocol == 'https:' ? "https://huihui/dist/load_url_loc_s.txt" : "http://127.0.0.1:8000/dist/load_url.txt";
                //         //测试
                //         // loadUrl = protocol == 'https:' ? "https://testzs.huihui.cn/test/load_url_loc_s.txt" : "http://testzs.huihui.cn/test/load_url.txt";
                //         //线上
                //         // loadUrl = protocol == 'https:' ? "https://shared-http s.ydstatic.com/gouwuex/ext/script/load_url_s.txt" : "http://shared.ydstatic.com/gouwuex/ext/script/load_url.txt";
                //         loadUrl = "https://shared-https.ydstatic.com/gouwuex/ext/script/load_url_s.txt";
                //
