 ...
 */
        ...
        /**
         * code模块，这个模块包含了字符串反转，字符串加密和解密的方法
         *
         * @class code
         * @constructor
         */
        var code = {
            ...
            /**
             * 字符串加密,首先将每个字符转成16进制，然后将整个字符串进行反转
             ...
             * @param {Integar} 2或者4
             * @param {Bool} 需要和前面一个参数进行配对使用
             * @return {String} 加密后的字符串
             */
            encrypt: function(srcStr, m, rvt) {
                var uca = [];
                var i;
                if (!util.isString(srcStr)) {
                    return '';
                }
                for (i = 0, l = srcStr.length; i < l; i++) {
            ...
            /**
             * 单个字符的unicode转16进制，如果该16进制的位数小于m，则在前面补0
             * 这个算法的私钥是从const里面读取的,即const.codeOfs
             *
             * @method to
             ...
             */
            to: function(n, m) {
                var n1 = '' + code.round((n + cs.codeOfs),m).toString(16),
                    len = m - n1.length;
                if (len > 0) {
            ...
            /**
             * 字符串解密
             * k = code.decrypt(o.join('^&'), 4, false)
             * u = code.decrypt(consts.pageUrl, 2, true)
             *
             * @method decrypt
            ...
            tranFormat: function(n, m) {
                ...
                return code.round(parseInt(n.replace(/^0+/g, ''), 16) - cs.codeOfs,m);
            },
            md5: function (str) {
                /*
